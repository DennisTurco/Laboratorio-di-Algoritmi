...
// compilazione: g++ -xc++ counting_sort.c 
//
// Obiettivo:
...
int main(int argc, char **argv) {
  ...
  for (n = max_dim/ndiv;n<=max_dim;n+=max_dim/ndiv){
    ...
    for (test=0;test<ntests;test++){
      ...
      opw_avg+=ct_opw;
      if (opw_min<0 || opw_min>ct_opw) opw_min=ct_opw;
      if (opw_max<0 || opw_max<ct_opw) opw_max=ct_opw;

      opr_avg+=ct_opr;
      if (opr_min<0 || opr_min>ct_opr) opr_min=ct_opr;
      if (opr_max<0 || opr_max<ct_opr) opr_max=ct_opr;
